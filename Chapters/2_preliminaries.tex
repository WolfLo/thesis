...
&= \sum_{t=0}^{H-1}\nabla_{\vtheta}\log\pi_{\vtheta}(a_{\tau,h}|s_{\tau,h}), \; \forall \tau\in\mathcal{T} \label{eq:logpolicy estimate}
\end{align}
...
\begin{algorithm}[t]
	...
	\label{alg:PG}
	\begin{algorithmic}[1]
	\State Initialize policy parameters $\vtheta^0$
	...
	\EndFor \\
	\Return policy parameters $\vtheta^{T_{max}}$
	\end{algorithmic}
\end{algorithm}
...
introduced in the early 1990s by Williams \cite{williams1992simple}, and include the famous REINFORCE algorithm. These methods build upon the policy gradient formulation given by the second equation \ref{eq:trajectoryPG}, which, in the same way of equation \ref{eq:reinforcetrick}, has been rewritten by applying the so called \emph{REINFORCE trick}, also called \emph{likelihood ratio} trick. As we have seen in equation \ref{eq:logpolicyestimate}, the policy gradient (eq. \ref{eq:trajectoryPG}) can be approximated by using a sum over the sampled trajectories. In a similar way, we can estimate $\Rew(\tau)$ in a Monte-Carlo fashion. However, such estimates suffer of very large variance. The variance can be reduced by introducing a baseline $b$ for the trajectory reward $\Rew(\tau)$, \ie

\begin{align}
\nabla_{\vtheta}J(\vtheta) = \myExpu{\tau\sim p_{\vtheta}}{\nabla_{\vtheta}\log p_{\vtheta}(\tau)\left(\Rew(\tau)-b\right)} \label{eq:trajectoryPG}
\end{align}

The estimator with baseline is still unbiased. This parameter can be chosen arbitrarily in order to minimize the variance. The variance-minimizing baseline is usually referred to as \emph{optimal baseline}.